...
class SimpleCalendar:
    ...
    async def start_calendar(
        ...
        inline_kb.insert(InlineKeyboardButton(
            "Месяц", callback_data=calendar_callback.new("CURR-MONTH", year, month, 0)))
        inline_kb.insert(InlineKeyboardButton(
            ">", callback_data=calendar_callback.new("NEXT-MONTH", year, month, 0)
        ))
        #inline_kb.add(CANCEL)
        mainmenubackbtn = InlineKeyboardButton('◀️ Назад', callback_data=floornum)
        inline_kb.row(mainmenubackbtn)
        return inline_kb

    async def process_selection(self, query: CallbackQuery, data: CallbackData, floornum) -> tuple:
        """
        ...
        if data['act'] == "PREV-YEAR":
            ...
            await query.message.edit_reply_markup(await self.start_calendar(int(prev_date.year), int(prev_date.month), floornum))
        # user navigates to next year, editing message with new calendar
        if data['act'] == "NEXT-YEAR":
            next_date = temp_date + timedelta(days=365)
            await query.message.edit_reply_markup(await self.start_calendar(int(next_date.year), int(next_date.month), floornum))
